...
Which model or models provide the best fit to the data, and the implications for the processes structuring ecological systems, has been a regular topic of debate (e.g., McGill 2003, Volkov et al. 2004, Ulrich et al. 2010, White et al. 2012). Most comparisons of the different models: 1) focus on a small subset of available models (typically two; e.g., McGill 2003, Volkov et al. 2004, White et al. 2012); 2) use a single dataset (e.g., McGill 2003, Volkov et al. 2004); or 3) fail to use the most appropriate modern statistical methods (e.g., Ulrich et al. 2010). This makes it difficult to draw general conclusions about which, if any, models provide the best empirical fit to species abundance distributions.
...
The untruncated logseries had the best model fit in the majority of cases (Figure 1).  It provided the most frequent best fit for the BBS, CBC, FIA, MCDB, and NABA datasets. The Poisson lognormal provided the best model fit for the Gentry data, with the untruncated logseries coming in a close second (Figure 2).  The truncated lognormal performed the worst for the combined datasets (Figure 1) (zero sites, not graphed), and was never the best fit for the BBS, CBC, Gentry, and NABA datasets (Figure 2)(zero sites, not graphed).  The negative binomial distribution failed to converge for a small number of sites (get actual numbers).

Distributions of AICc weights provides a more complete picture of the overall fits of the different models. The peaks of the AICc weights overlapped for the truncated logseries, negative binomial, and Poisson lognormal (Figure 3). The negative binomial distribution was more right skewed-meaning resulting  in a higher mean AICc weight that the other models in this mode <!-- check this statement -->. The Poisson lognormal had an additional smaller peak around 1, indicating that it had very good model fit in the cases where it was the winning model (Figure 3).  The majority of the AICc weights for the geometric series were near zero (Figure 3).

Although the truncated logseries and untruncated logseries do not have much overlap for AICc weights (Figure 3), and the truncated logseries was never the winning model (Figures 1, 2), the log-likelihood values have almost complete congruence (Figure 4).  This indicates that both the truncated and untruncated logseries fit equally well to the data, and that the difference in AICc values is due to the penalty for the additional parameter (the upper truncation). All of the models but the geometric series have very similar log-likelihood values, indicating very similar goodnesses of fit of models to data (Figure 4).
